...
def split_slice(sl, sheet, margin, tab_w):
    ...
    holes = [Polygon(r) for poly in sl.profile.geoms for r in poly.interiors]
    ...
    for i in range(n):
        ...
        pieces_geom.append(sl.profile.intersection(region))
    # tabs on each internal cut: alternate sides; add to one piece, subtract from the neighbour
    for i, c in enumerate(cuts[1:-1]):
        line = sl.profile.intersection(box(c - 0.01, -big, c + 0.01, big) if axis == 0 else box(-big, c - 0.01, big, c + 0.01))
        if line.is_empty:
            continue
        ...
        for j, cc in enumerate(np.linspace(lo2, hi2, n_tabs + 2)[1:-1]):
            ...
            for d in (0, tab_w, -tab_w, 2 * tab_w, -2 * tab_w):
                ...
                if sl.profile.contains(room) and all(room.disjoint(q) for q in tabs):
                    break
            else:
                continue
    ...
    sl.pieces = [Piece(f"{sl.label}-{i + 1}", as_multi(g), sl) for i, g in enumerate(pieces_geom) if not g.is_empty]
